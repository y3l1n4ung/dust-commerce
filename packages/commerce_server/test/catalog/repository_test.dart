...
void main() {
  ...
  setUp(() async {
    ...
    catalog = CatalogRepository(database.executor);
  ...
  group('listPublished', () {
    test('returns only published products', () async {
      final result = await catalog.listPublished(10, 0);
    ...
    test('pages, so a large catalogue does not arrive at once', () async {
      final first = await catalog.listPublished(1, 0);
      final second = await catalog.listPublished(1, 1);
    ...
    test('counts what it would page through', () async {
      expect(ok(await catalog.countPublished()), 2);
    });
  });

  group('findByHandle', () {
    test('finds a published product', () async {
      final row = ok(await catalog.findByHandle('t-shirt'));
    ...
    test('does not leak a draft, even to a caller who knows the handle',
        () async {
      expect(ok(await catalog.findByHandle('secret-hoodie')), isNull);
    });

    test('returns null for a handle nobody has', () async {
      expect(ok(await catalog.findByHandle('nothing')), isNull);
    });
  });

  group('variantsOf', () {
    test('returns the variants priced in the asked-for currency', () async {
      final rows = ok(await catalog.variantsOf('prod_shirt', 'usd'));
    ...
    test('drops a variant with no price in that currency', () async {
      final rows = ok(await catalog.variantsOf('prod_shirt', 'eur'));
    ...
    test('returns nothing for a currency nobody is priced in', () async {
      expect(ok(await catalog.variantsOf('prod_shirt', 'gbp')), isEmpty);
    });
  });

  group('findVariant', () {
    test('finds one variant with its price', () async {
      final row = ok(await catalog.findVariant('var_small', 'usd'));
    ...
    test('returns null when the variant is not sold in that currency',
        () async {
      expect(ok(await catalog.findVariant('var_large', 'eur')), isNull);
    });
  });
}
